import { useEffect, useMemo, useState } from "react";
import {
  View,
  Text,
  TouchableOpacity,
  ScrollView,
  TextInput,
  FlatList,
  Alert,
  Linking,
  ActivityIndicator,
} from "react-native";
import { useTheme } from "../../theme/theme";
import { useSafeAreaInsets } from "react-native-safe-area-context";
import { useProfile } from "../../store/profile.store";
import { useAuth } from "../../store/auth.store";
import Svg, { Circle } from "react-native-svg";
import {
  FileText,
  Edit3,
  Save,
  Plus,
  X,
  CheckCircle,
  Award,
  Download,
  Link as LinkIcon,
} from "lucide-react-native";
import {
  getCurriculoByUser,
  saveCurriculo,
} from "../../services/curriculo.service";
import { gerarCurriculoPDF } from "../../services/curriculoPdf.service";
import { Exp, Edu, Proj, Cert, Lnk } from "../../types/curriculo.types";

export default function CurriculoScreen() {
  const t = useTheme();
  const insets = useSafeAreaInsets();
  const { profile, setField, addSkill, removeSkill } = useProfile();
  const { user } = useAuth();

  const [editing, setEditing] = useState(true);

  const [name, setName] = useState(profile.name || "");
  const [email, setEmail] = useState(profile.email || "");
  const [role, setRole] = useState(profile.role || "");
  const [phone, setPhone] = useState(profile.phone || "");
  const [location, setLocation] = useState(profile.location || "");
  const [summary, setSummary] = useState(profile.bio || "");

  const [skillsInput, setSkillsInput] = useState("");
  const [experiences, setExperiences] = useState<Exp[]>([]);
  const [edu, setEdu] = useState<Edu[]>([]);
  const [projects, setProjects] = useState<Proj[]>([]);
  const [certs, setCerts] = useState<Cert[]>([]);
  const [links, setLinks] = useState<Lnk[]>([]);

  const [expDraft, setExpDraft] = useState<Exp>({
    role: "",
    company: "",
    start: "",
    end: "",
    desc: "",
  });
  const [eduDraft, setEduDraft] = useState<Edu>({
    course: "",
    school: "",
    start: "",
    end: "",
  });
  const [projDraft, setProjDraft] = useState<Proj>({
    name: "",
    link: "",
    desc: "",
  });
  const [certDraft, setCertDraft] = useState<Cert>({
    name: "",
    org: "",
    year: "",
  });
  const [linkDraft, setLinkDraft] = useState<Lnk>({ label: "", url: "" });

  const [loading, setLoading] = useState(false);
  const [saving, setSaving] = useState(false);
  const [curriculoId, setCurriculoId] = useState<number | null>(null);

  const userId =
    user && Number(user.id) && !Number.isNaN(Number(user.id))
      ? Number(user.id)
      : null;

  useEffect(() => {
    if (!userId) return;

    const load = async () => {
      try {
        setLoading(true);
        const cv = await getCurriculoByUser(userId);
        if (cv) {
          setCurriculoId(cv.idCurriculo);
          if (cv.summary) setSummary(cv.summary);
          if (Array.isArray(cv.experiences))
            setExperiences(cv.experiences as Exp[]);
          if (Array.isArray(cv.edu)) setEdu(cv.edu as Edu[]);
          if (Array.isArray(cv.projects)) setProjects(cv.projects as Proj[]);
          if (Array.isArray(cv.certs)) setCerts(cv.certs as Cert[]);
          if (Array.isArray(cv.links)) setLinks(cv.links as Lnk[]);
          const existingSkills = profile.skills || [];
          cv.skills.forEach((s: string) => {
            if (!existingSkills.includes(s)) addSkill(s);
          });
        }
      } catch {
      } finally {
        setLoading(false);
      }
    };

    load();
  }, [userId]);

  const completeness = useMemo(() => {
    let pts = 0;
    let total = 0;
    const bump = (v: boolean) => {
      total += 1;
      if (v) pts += 1;
    };

    bump(Boolean(name.trim()));
    bump(Boolean(email.trim()));
    bump(Boolean(role.trim()));
    bump(Boolean(phone.trim()));
    bump(Boolean(location.trim()));

    bump(summary.trim().length >= 60);

    bump((profile.skills?.length || 0) >= 3);

    bump(experiences.length > 0);

    bump(edu.length > 0);

    bump(projects.length > 0 || certs.length > 0);

    bump(links.length > 0);

    const pct = Math.round((pts / total) * 100);
    return Math.max(0, Math.min(100, pct));
  }, [
    name,
    email,
    role,
    phone,
    location,
    summary,
    profile.skills,
    experiences,
    edu,
    projects,
    certs,
    links,
  ]);

  const suggestions = useMemo(() => {
    const s: string[] = [];
    const skillsCount = profile.skills?.length || 0;

    if (!name.trim()) s.push("Informe seu nome completo.");
    if (!email.trim()) s.push("Adicione um e-mail de contato.");
    if (!role.trim())
      s.push(
        "Defina um cargo ou objetivo profissional (ex.: Analista de Dados Júnior)."
      );
    if (!phone.trim()) s.push("Inclua um telefone para contato.");
    if (!location.trim())
      s.push("Preencha sua localização (cidade e estado).");

    if (summary.trim().length < 60)
      s.push(
        "Escreva um resumo com pelo menos 3–4 linhas sobre quem você é e o que busca."
      );
    if (skillsCount < 3)
      s.push(`Adicione pelo menos ${3 - skillsCount} habilidade(s) chave.`);

    if (experiences.length === 0)
      s.push(
        "Inclua ao menos uma experiência (emprego, estágio ou trabalho voluntário)."
      );

    if (edu.length === 0)
      s.push("Inclua ao menos uma formação (curso, graduação ou extensão).");

    if (projects.length === 0 && certs.length === 0)
      s.push(
        "Adicione pelo menos um projeto relevante ou uma certificação."
      );

    if (links.length === 0)
      s.push("Inclua um link profissional (ex.: LinkedIn ou GitHub).");

    return s;
  }, [
    name,
    email,
    role,
    phone,
    location,
    summary,
    profile.skills,
    experiences.length,
    edu.length,
    projects.length,
    certs.length,
    links.length,
  ]);

  const size = 140;
  const stroke = 12;
  const r = (size - stroke) / 2;
  const c = 2 * Math.PI * r;

  const persistProfileBasics = () => {
    setField("name", name);
    setField("email", email);
    setField("role", role);
    setField("phone", phone);
    setField("location", location);
    setField("bio", summary);
  };

  const addExperience = () => {
    if (!expDraft.role.trim() || !expDraft.company.trim()) return;
    setExperiences((s) => [...s, expDraft]);
    setExpDraft({ role: "", company: "", start: "", end: "", desc: "" });
  };

  const addEducation = () => {
    if (!eduDraft.course.trim() || !eduDraft.school.trim()) return;
    setEdu((s) => [...s, eduDraft]);
    setEduDraft({ course: "", school: "", start: "", end: "" });
  };

  const addProject = () => {
    if (!projDraft.name.trim()) return;
    setProjects((s) => [...s, projDraft]);
    setProjDraft({ name: "", link: "", desc: "" });
  };

  const addCert = () => {
    if (!certDraft.name.trim()) return;
    setCerts((s) => [...s, certDraft]);
    setCertDraft({ name: "", org: "", year: "" });
  };

  const addLink = () => {
    if (!linkDraft.label.trim() || !linkDraft.url.trim()) return;
    setLinks((s) => [...s, linkDraft]);
    setLinkDraft({ label: "", url: "" });
  };

  const removeAt = <T,>(arr: T[], i: number) =>
    arr.filter((_, idx) => idx !== i);

  const handleGeneratePdf = async () => {
    persistProfileBasics();
    await gerarCurriculoPDF({
      name,
      email,
      role,
      phone,
      location,
      summary,
      skills: profile.skills || [],
      experiences,
      education: edu,
      projects,
      certs,
      links,
      completeness,
    });
  };

  const initials = name
    .split(" ")
    .map((p) => p[0])
    .slice(0, 2)
    .join("")
    .toUpperCase();

  return (
    <ScrollView
      showsVerticalScrollIndicator={false}
      contentContainerStyle={{
        padding: t.spacing.lg,
        gap: t.spacing.lg,
        paddingBottom: insets.bottom + 120,
        maxWidth: 900,
        alignSelf: "center",
        width: "100%",
      }}
      style={{ flex: 1, backgroundColor: t.colors.background }}
    >
      <View style={{ alignItems: "center", gap: t.spacing.sm }}>
        <View
          style={{
            width: 96,
            height: 96,
            borderRadius: 999,
            alignItems: "center",
            justifyContent: "center",
            backgroundColor: "rgba(78,242,195,0.12)",
            borderWidth: 1,
            borderColor: t.colors.border,
          }}
        >
          {initials ? (
            <Text
              style={{
                color: t.colors.primary,
                fontSize: 32,
                fontWeight: "900",
              }}
            >
              {initials}
            </Text>
          ) : (
            <FileText color={t.colors.primary} size={40} />
          )}
        </View>
        <Text
          style={{ color: t.colors.text, fontSize: 22, fontWeight: "800" }}
        >
          Currículo Inteligente
        </Text>
        <Text style={{ color: t.colors.textMuted }}>
          Preencha os dados e gere o PDF
        </Text>
        <TouchableOpacity
          onPress={async () => {
            if (!editing) {
              setEditing(true);
              return;
            }
            if (!userId) {
              Alert.alert("Erro", "Usuário não identificado.");
              return;
            }
            try {
              setSaving(true);
              persistProfileBasics();
              const skills = profile.skills || [];
              const id = await saveCurriculo(
                userId,
                {
                  summary,
                  skills,
                  experiences,
                  edu,
                  projects,
                  certs,
                  links,
                },
                curriculoId
              );
              setCurriculoId(id);
              setEditing(false);
              Alert.alert("Sucesso", "Currículo salvo.");
            } catch {
              Alert.alert("Erro", "Não foi possível salvar o currículo.");
            } finally {
              setSaving(false);
            }
          }}
          disabled={saving}
          style={{
            marginTop: t.spacing.xs,
            backgroundColor: t.colors.glass,
            paddingHorizontal: 12,
            paddingVertical: 8,
            borderRadius: t.radius.pill,
            borderWidth: 1,
            borderColor: t.colors.border,
            flexDirection: "row",
            alignItems: "center",
            gap: 8,
            opacity: saving ? 0.7 : 1,
          }}
        >
          {editing ? (
            <Save color={t.colors.text} size={16} />
          ) : (
            <Edit3 color={t.colors.text} size={16} />
          )}
          <Text style={{ color: t.colors.text, fontWeight: "700" }}>
            {editing ? (saving ? "Salvando..." : "Salvar") : "Editar"}
          </Text>
        </TouchableOpacity>
        {loading && (
          <View style={{ marginTop: t.spacing.xs, alignItems: "center" }}>
            <ActivityIndicator size="small" color={t.colors.primary} />
            <Text
              style={{
                color: t.colors.textMuted,
                fontSize: 12,
                marginTop: 4,
              }}
            >
              Carregando currículo salvo...
            </Text>
          </View>
        )}
      </View>

      <View
        style={{
          backgroundColor: t.colors.glass,
          borderRadius: t.radius.lg,
          borderWidth: 1,
          borderColor: t.colors.border,
          padding: t.spacing.lg,
          gap: t.spacing.sm,
        }}
      >
        <Text
          style={{
            color: t.colors.text,
            fontSize: 16,
            fontWeight: "800",
          }}
        >
          Informações pessoais
        </Text>

        <View style={{ gap: t.spacing.sm }}>
          <View
            style={{
              flexDirection: "row",
              gap: t.spacing.sm,
              flexWrap: "wrap",
            }}
          >
            <TextInput
              editable={editing}
              value={name}
              onChangeText={setName}
              placeholder="Nome completo"
              placeholderTextColor={t.colors.tabInactive}
              style={{
                flex: 1,
                minWidth: "100%",
                backgroundColor: "#10152A",
                color: t.colors.text,
                borderRadius: t.radius.md,
                borderWidth: 1,
                borderColor: t.colors.border,
                paddingHorizontal: 12,
                height: 44,
              }}
            />
          </View>
          <View
            style={{
              flexDirection: "row",
              gap: t.spacing.sm,
              flexWrap: "wrap",
            }}
          >
            <TextInput
              editable={editing}
              value={email}
              onChangeText={setEmail}
              placeholder="Email"
              keyboardType="email-address"
              autoCapitalize="none"
              placeholderTextColor={t.colors.tabInactive}
              style={{
                flex: 1,
                minWidth: "100%",
                backgroundColor: "#10152A",
                color: t.colors.text,
                borderRadius: t.radius.md,
                borderWidth: 1,
                borderColor: t.colors.border,
                paddingHorizontal: 12,
                height: 44,
              }}
            />
          </View>
          <View
            style={{
              flexDirection: "row",
              gap: t.spacing.sm,
              flexWrap: "wrap",
            }}
          >
            <TextInput
              editable={editing}
              value={role}
              onChangeText={setRole}
              placeholder="Cargo/Objetivo"
              placeholderTextColor={t.colors.tabInactive}
              style={{
                flex: 1,
                minWidth: "100%",
                backgroundColor: "#10152A",
                color: t.colors.text,
                borderRadius: t.radius.md,
                borderWidth: 1,
                borderColor: t.colors.border,
                paddingHorizontal: 12,
                height: 44,
              }}
            />
          </View>
          <View
            style={{
              flexDirection: "row",
              gap: t.spacing.sm,
              flexWrap: "wrap",
            }}
          >
            <TextInput
              editable={editing}
              value={phone}
              onChangeText={setPhone}
              placeholder="Telefone"
              placeholderTextColor={t.colors.tabInactive}
              style={{
                flex: 1,
                minWidth: 140,
                backgroundColor: "#10152A",
                color: t.colors.text,
                borderRadius: t.radius.md,
                borderWidth: 1,
                borderColor: t.colors.border,
                paddingHorizontal: 12,
                height: 44,
              }}
            />
            <TextInput
              editable={editing}
              value={location}
              onChangeText={setLocation}
              placeholder="Localização"
              placeholderTextColor={t.colors.tabInactive}
              style={{
                flex: 1,
                minWidth: 140,
                backgroundColor: "#10152A",
                color: t.colors.text,
                borderRadius: t.radius.md,
                borderWidth: 1,
                borderColor: t.colors.border,
                paddingHorizontal: 12,
                height: 44,
              }}
            />
          </View>
          <TextInput
            editable={editing}
            value={summary}
            onChangeText={setSummary}
            placeholder="Resumo (mín. 60 caracteres para pontuar mais)"
            placeholderTextColor={t.colors.tabInactive}
            multiline
            style={{
              backgroundColor: "#10152A",
              color: t.colors.text,
              borderRadius: t.radius.md,
              borderWidth: 1,
              borderColor: t.colors.border,
              padding: t.spacing.md,
              minHeight: 90,
            }}
          />
        </View>
      </View>

      <View
        style={{
          backgroundColor: t.colors.surfaceAlt,
          borderRadius: t.radius.lg,
          borderWidth: 1,
          borderColor: t.colors.border,
          padding: t.spacing.lg,
          gap: t.spacing.sm,
        }}
      >
        <Text
          style={{
            color: t.colors.text,
            fontSize: 16,
            fontWeight: "800",
          }}
        >
          Habilidades
        </Text>
        {editing && (
          <View
            style={{
              flexDirection: "row",
              gap: t.spacing.sm,
              flexWrap: "wrap",
            }}
          >
            <TextInput
              value={skillsInput}
              onChangeText={setSkillsInput}
              placeholder="Adicionar habilidade"
              placeholderTextColor={t.colors.tabInactive}
              style={{
                flex: 1,
                minWidth: 160,
                height: 44,
                backgroundColor: "#10152A",
                color: t.colors.text,
                borderRadius: t.radius.md,
                borderWidth: 1,
                borderColor: t.colors.border,
                paddingHorizontal: 12,
              }}
            />
            <TouchableOpacity
              onPress={() => {
                const v = skillsInput.trim();
                if (v) {
                  addSkill(v);
                  setSkillsInput("");
                }
              }}
              style={{
                height: 44,
                paddingHorizontal: 14,
                borderRadius: t.radius.md,
                backgroundColor: t.colors.primary,
                alignItems: "center",
                justifyContent: "center",
                flexDirection: "row",
                gap: 6,
              }}
            >
              <Plus color="#0B0D13" />
              <Text style={{ color: "#0B0D13", fontWeight: "800" }}>Add</Text>
            </TouchableOpacity>
          </View>
        )}
        <FlatList
          data={profile.skills}
          keyExtractor={(i) => i}
          horizontal
          showsHorizontalScrollIndicator={false}
          contentContainerStyle={{ gap: t.spacing.sm, paddingVertical: 2 }}
          renderItem={({ item }) => (
            <View
              style={{
                flexDirection: "row",
                alignItems: "center",
                backgroundColor: "#1A2035",
                paddingHorizontal: 12,
                paddingVertical: 8,
                borderRadius: t.radius.pill,
                borderWidth: 1,
                borderColor: t.colors.border,
              }}
            >
              <Text
                style={{ color: t.colors.primary, fontWeight: "700" }}
              >
                {item}
              </Text>
              {editing && (
                <TouchableOpacity
                  onPress={() => removeSkill(item)}
                  style={{ marginLeft: 8 }}
                >
                  <X color={t.colors.tabInactive} size={16} />
                </TouchableOpacity>
              )}
            </View>
          )}
          ListEmptyComponent={
            <Text style={{ color: t.colors.textMuted }}>
              Sem habilidades
            </Text>
          }
        />
      </View>

      <View
        style={{
          backgroundColor: t.colors.glass,
          borderRadius: t.radius.lg,
          borderWidth: 1,
          borderColor: t.colors.border,
          padding: t.spacing.lg,
          gap: t.spacing.md,
        }}
      >
        <Text
          style={{
            color: t.colors.text,
            fontSize: 16,
            fontWeight: "800",
          }}
        >
          Experiência
        </Text>
        {editing && (
          <View style={{ gap: t.spacing.sm }}>
            <TextInput
              value={expDraft.role}
              onChangeText={(v) => setExpDraft({ ...expDraft, role: v })}
              placeholder="Cargo"
              placeholderTextColor={t.colors.tabInactive}
              style={{
                backgroundColor: "#10152A",
                color: t.colors.text,
                borderRadius: t.radius.md,
                borderWidth: 1,
                borderColor: t.colors.border,
                paddingHorizontal: 12,
                height: 44,
              }}
            />
            <TextInput
              value={expDraft.company}
              onChangeText={(v) =>
                setExpDraft({ ...expDraft, company: v })
              }
              placeholder="Empresa"
              placeholderTextColor={t.colors.tabInactive}
              style={{
                backgroundColor: "#10152A",
                color: t.colors.text,
                borderRadius: t.radius.md,
                borderWidth: 1,
                borderColor: t.colors.border,
                paddingHorizontal: 12,
                height: 44,
              }}
            />
            <View
              style={{
                flexDirection: "row",
                gap: t.spacing.sm,
                flexWrap: "wrap",
              }}
            >
              <TextInput
                value={expDraft.start}
                onChangeText={(v) =>
                  setExpDraft({ ...expDraft, start: v })
                }
                placeholder="Início (ex.: 01/2024)"
                placeholderTextColor={t.colors.tabInactive}
                style={{
                  flex: 1,
                  minWidth: 140,
                  backgroundColor: "#10152A",
                  color: t.colors.text,
                  borderRadius: t.radius.md,
                  borderWidth: 1,
                  borderColor: t.colors.border,
                  paddingHorizontal: 12,
                  height: 44,
                }}
              />
              <TextInput
                value={expDraft.end}
                onChangeText={(v) => setExpDraft({ ...expDraft, end: v })}
                placeholder="Fim (ou Atual)"
                placeholderTextColor={t.colors.tabInactive}
                style={{
                  flex: 1,
                  minWidth: 140,
                  backgroundColor: "#10152A",
                  color: t.colors.text,
                  borderRadius: t.radius.md,
                  borderWidth: 1,
                  borderColor: t.colors.border,
                  paddingHorizontal: 12,
                  height: 44,
                }}
              />
            </View>
            <TextInput
              value={expDraft.desc}
              onChangeText={(v) => setExpDraft({ ...expDraft, desc: v })}
              placeholder="Descrição (impacto, resultados, techs)"
              placeholderTextColor={t.colors.tabInactive}
              multiline
              style={{
                backgroundColor: "#10152A",
                color: t.colors.text,
                borderRadius: t.radius.md,
                borderWidth: 1,
                borderColor: t.colors.border,
                padding: t.spacing.md,
              }}
            />
            <TouchableOpacity
              onPress={addExperience}
              style={{
                alignSelf: "flex-start",
                backgroundColor: t.colors.primary,
                paddingHorizontal: 14,
                paddingVertical: 10,
                borderRadius: t.radius.md,
              }}
            >
              <Text style={{ color: "#0B0D13", fontWeight: "800" }}>
                Adicionar experiência
              </Text>
            </TouchableOpacity>
          </View>
        )}
        <View style={{ gap: t.spacing.sm }}>
          {experiences.map((e, i) => (
            <View
              key={`${e.role}-${i}`}
              style={{
                backgroundColor: "#1A2035",
                borderRadius: t.radius.md,
                borderWidth: 1,
                borderColor: t.colors.border,
                padding: t.spacing.md,
              }}
            >
              <View
                style={{
                  flexDirection: "row",
                  justifyContent: "space-between",
                  alignItems: "center",
                }}
              >
                <View style={{ flex: 1, paddingRight: 8 }}>
                  <Text
                    style={{
                      color: t.colors.text,
                      fontWeight: "800",
                    }}
                  >
                    {e.role}
                  </Text>
                  <Text style={{ color: t.colors.textMuted }}>
                    {e.company} •{" "}
                    {[e.start, e.end].filter(Boolean).join(" — ")}
                  </Text>
                </View>
                {editing && (
                  <TouchableOpacity
                    onPress={() =>
                      setExperiences(removeAt(experiences, i))
                    }
                  >
                    <X color={t.colors.tabInactive} />
                  </TouchableOpacity>
                )}
              </View>
              {e.desc ? (
                <Text
                  style={{ color: t.colors.text, marginTop: 6 }}
                >
                  {e.desc}
                </Text>
              ) : null}
            </View>
          ))}
          {!experiences.length && (
            <Text style={{ color: t.colors.textMuted }}>
              Sem experiências
            </Text>
          )}
        </View>
      </View>

      <View
        style={{
          backgroundColor: t.colors.surfaceAlt,
          borderRadius: t.radius.lg,
          borderWidth: 1,
          borderColor: t.colors.border,
          padding: t.spacing.lg,
          gap: t.spacing.md,
        }}
      >
        <Text
          style={{
            color: t.colors.text,
            fontSize: 16,
            fontWeight: "800",
          }}
        >
          Formação
        </Text>
        {editing && (
          <View style={{ gap: t.spacing.sm }}>
            <TextInput
              value={eduDraft.course}
              onChangeText={(v) =>
                setEduDraft({ ...eduDraft, course: v })
              }
              placeholder="Curso"
              placeholderTextColor={t.colors.tabInactive}
              style={{
                backgroundColor: "#10152A",
                color: t.colors.text,
                borderRadius: t.radius.md,
                borderWidth: 1,
                borderColor: t.colors.border,
                paddingHorizontal: 12,
                height: 44,
              }}
            />
            <TextInput
              value={eduDraft.school}
              onChangeText={(v) =>
                setEduDraft({ ...eduDraft, school: v })
              }
              placeholder="Instituição"
              placeholderTextColor={t.colors.tabInactive}
              style={{
                backgroundColor: "#10152A",
                color: t.colors.text,
                borderRadius: t.radius.md,
                borderWidth: 1,
                borderColor: t.colors.border,
                paddingHorizontal: 12,
                height: 44,
              }}
            />
            <View
              style={{
                flexDirection: "row",
                gap: t.spacing.sm,
                flexWrap: "wrap",
              }}
            >
              <TextInput
                value={eduDraft.start}
                onChangeText={(v) =>
                  setEduDraft({ ...eduDraft, start: v })
                }
                placeholder="Início"
                placeholderTextColor={t.colors.tabInactive}
                style={{
                  flex: 1,
                  minWidth: 140,
                  backgroundColor: "#10152A",
                  color: t.colors.text,
                  borderRadius: t.radius.md,
                  borderWidth: 1,
                  borderColor: t.colors.border,
                  paddingHorizontal: 12,
                  height: 44,
                }}
              />
              <TextInput
                value={eduDraft.end}
                onChangeText={(v) =>
                  setEduDraft({ ...eduDraft, end: v })
                }
                placeholder="Conclusão"
                placeholderTextColor={t.colors.tabInactive}
                style={{
                  flex: 1,
                  minWidth: 140,
                  backgroundColor: "#10152A",
                  color: t.colors.text,
                  borderRadius: t.radius.md,
                  borderWidth: 1,
                  borderColor: t.colors.border,
                  paddingHorizontal: 12,
                  height: 44,
                }}
              />
            </View>
            <TouchableOpacity
              onPress={addEducation}
              style={{
                alignSelf: "flex-start",
                backgroundColor: t.colors.primary,
                paddingHorizontal: 14,
                paddingVertical: 10,
                borderRadius: t.radius.md,
              }}
            >
              <Text style={{ color: "#0B0D13", fontWeight: "800" }}>
                Adicionar formação
              </Text>
            </TouchableOpacity>
          </View>
        )}
        <View style={{ gap: t.spacing.sm }}>
          {edu.map((e, i) => (
            <View
              key={`${e.course}-${i}`}
              style={{
                backgroundColor: "#1A2035",
                borderRadius: t.radius.md,
                borderWidth: 1,
                borderColor: t.colors.border,
                padding: t.spacing.md,
                flexDirection: "row",
                justifyContent: "space-between",
                alignItems: "center",
              }}
            >
              <View style={{ flex: 1, paddingRight: 8 }}>
                <Text
                  style={{
                    color: t.colors.text,
                    fontWeight: "800",
                  }}
                >
                  {e.course}
                </Text>
                <Text style={{ color: t.colors.textMuted }}>
                  {e.school} •{" "}
                  {[e.start, e.end].filter(Boolean).join(" — ")}
                </Text>
              </View>
              {editing && (
                <TouchableOpacity
                  onPress={() => setEdu(removeAt(edu, i))}
                >
                  <X color={t.colors.tabInactive} />
                </TouchableOpacity>
              )}
            </View>
          ))}
          {!edu.length && (
            <Text style={{ color: t.colors.textMuted }}>
              Sem formação
            </Text>
          )}
        </View>
      </View>

      <View
        style={{
          backgroundColor: t.colors.glass,
          borderRadius: t.radius.lg,
          borderWidth: 1,
          borderColor: t.colors.border,
          padding: t.spacing.lg,
          gap: t.spacing.md,
        }}
      >
        <Text
          style={{
            color: t.colors.text,
            fontSize: 16,
            fontWeight: "800",
          }}
        >
          Projetos
        </Text>
        {editing && (
          <View style={{ gap: t.spacing.sm }}>
            <TextInput
              value={projDraft.name}
              onChangeText={(v) =>
                setProjDraft({ ...projDraft, name: v })
              }
              placeholder="Nome do projeto"
              placeholderTextColor={t.colors.tabInactive}
              style={{
                backgroundColor: "#10152A",
                color: t.colors.text,
                borderRadius: t.radius.md,
                borderWidth: 1,
                borderColor: t.colors.border,
                paddingHorizontal: 12,
                height: 44,
              }}
            />
            <TextInput
              value={projDraft.link}
              onChangeText={(v) =>
                setProjDraft({ ...projDraft, link: v })
              }
              placeholder="Link (GitHub, site)"
              placeholderTextColor={t.colors.tabInactive}
              autoCapitalize="none"
              style={{
                backgroundColor: "#10152A",
                color: t.colors.text,
                borderRadius: t.radius.md,
                borderWidth: 1,
                borderColor: t.colors.border,
                paddingHorizontal: 12,
                height: 44,
              }}
            />
            <TextInput
              value={projDraft.desc}
              onChangeText={(v) =>
                setProjDraft({ ...projDraft, desc: v })
              }
              placeholder="Descrição"
              placeholderTextColor={t.colors.tabInactive}
              multiline
              style={{
                backgroundColor: "#10152A",
                color: t.colors.text,
                borderRadius: t.radius.md,
                borderWidth: 1,
                borderColor: t.colors.border,
                padding: t.spacing.md,
              }}
            />
            <TouchableOpacity
              onPress={addProject}
              style={{
                alignSelf: "flex-start",
                backgroundColor: t.colors.primary,
                paddingHorizontal: 14,
                paddingVertical: 10,
                borderRadius: t.radius.md,
              }}
            >
              <Text style={{ color: "#0B0D13", fontWeight: "800" }}>
                Adicionar projeto
              </Text>
            </TouchableOpacity>
          </View>
        )}
        <View style={{ gap: t.spacing.sm }}>
          {projects.map((p, i) => (
            <View
              key={`${p.name}-${i}`}
              style={{
                backgroundColor: "#1A2035",
                borderRadius: t.radius.md,
                borderWidth: 1,
                borderColor: t.colors.border,
                padding: t.spacing.md,
              }}
            >
              <View
                style={{
                  flexDirection: "row",
                  justifyContent: "space-between",
                  alignItems: "center",
                }}
              >
                <Text
                  style={{
                    color: t.colors.text,
                    fontWeight: "800",
                  }}
                >
                  {p.name}
                </Text>
                {editing && (
                  <TouchableOpacity
                    onPress={() => setProjects(removeAt(projects, i))}
                  >
                    <X color={t.colors.tabInactive} />
                  </TouchableOpacity>
                )}
              </View>
              {p.link ? (
                <TouchableOpacity
                  onPress={() => Linking.openURL(p.link!)}
                  style={{
                    flexDirection: "row",
                    alignItems: "center",
                    gap: 6,
                    marginTop: 6,
                  }}
                >
                  <LinkIcon color={t.colors.primary} size={16} />
                  <Text style={{ color: t.colors.primary }}>
                    {p.link}
                  </Text>
                </TouchableOpacity>
              ) : null}
              {p.desc ? (
                <Text
                  style={{ color: t.colors.text, marginTop: 6 }}
                >
                  {p.desc}
                </Text>
              ) : null}
            </View>
          ))}
          {!projects.length && (
            <Text style={{ color: t.colors.textMuted }}>
              Sem projetos
            </Text>
          )}
        </View>
      </View>

      <View
        style={{
          backgroundColor: t.colors.surfaceAlt,
          borderRadius: t.radius.lg,
          borderWidth: 1,
          borderColor: t.colors.border,
          padding: t.spacing.lg,
          gap: t.spacing.md,
        }}
      >
        <Text
          style={{
            color: t.colors.text,
            fontSize: 16,
            fontWeight: "800",
          }}
        >
          Certificações
        </Text>
        {editing && (
          <View style={{ gap: t.spacing.sm }}>
            <TextInput
              value={certDraft.name}
              onChangeText={(v) =>
                setCertDraft({ ...certDraft, name: v })
              }
              placeholder="Certificação"
              placeholderTextColor={t.colors.tabInactive}
              style={{
                backgroundColor: "#10152A",
                color: t.colors.text,
                borderRadius: t.radius.md,
                borderWidth: 1,
                borderColor: t.colors.border,
                paddingHorizontal: 12,
                height: 44,
              }}
            />
            <View
              style={{
                flexDirection: "row",
                gap: t.spacing.sm,
                flexWrap: "wrap",
              }}
            >
              <TextInput
                value={certDraft.org}
                onChangeText={(v) =>
                  setCertDraft({ ...certDraft, org: v })
                }
                placeholder="Instituição"
                placeholderTextColor={t.colors.tabInactive}
                style={{
                  flex: 1,
                  minWidth: 160,
                  backgroundColor: "#10152A",
                  color: t.colors.text,
                  borderRadius: t.radius.md,
                  borderWidth: 1,
                  borderColor: t.colors.border,
                  paddingHorizontal: 12,
                  height: 44,
                }}
              />
              <TextInput
                value={certDraft.year}
                onChangeText={(v) =>
                  setCertDraft({ ...certDraft, year: v })
                }
                placeholder="Ano"
                placeholderTextColor={t.colors.tabInactive}
                style={{
                  flex: 0.6,
                  minWidth: 90,
                  backgroundColor: "#10152A",
                  color: t.colors.text,
                  borderRadius: t.radius.md,
                  borderWidth: 1,
                  borderColor: t.colors.border,
                  paddingHorizontal: 12,
                  height: 44,
                }}
              />
            </View>
            <TouchableOpacity
              onPress={addCert}
              style={{
                alignSelf: "flex-start",
                backgroundColor: t.colors.primary,
                paddingHorizontal: 14,
                paddingVertical: 10,
                borderRadius: t.radius.md,
              }}
            >
              <Text style={{ color: "#0B0D13", fontWeight: "800" }}>
                Adicionar certificação
              </Text>
            </TouchableOpacity>
          </View>
        )}
        <View style={{ gap: t.spacing.sm }}>
          {certs.map((c, i) => (
            <View
              key={`${c.name}-${i}`}
              style={{
                backgroundColor: "#1A2035",
                borderRadius: t.radius.md,
                borderWidth: 1,
                borderColor: t.colors.border,
                padding: t.spacing.md,
                flexDirection: "row",
                justifyContent: "space-between",
                alignItems: "center",
              }}
            >
              <View style={{ flex: 1, paddingRight: 8 }}>
                <Text
                  style={{
                    color: t.colors.text,
                    fontWeight: "800",
                  }}
                >
                  {c.name}
                </Text>
                <Text style={{ color: t.colors.textMuted }}>
                  {[c.org, c.year].filter(Boolean).join(" • ")}
                </Text>
              </View>
              {editing && (
                <TouchableOpacity
                  onPress={() => setCerts(removeAt(certs, i))}
                >
                  <X color={t.colors.tabInactive} />
                </TouchableOpacity>
              )}
            </View>
          ))}
          {!certs.length && (
            <Text style={{ color: t.colors.textMuted }}>
              Sem certificações
            </Text>
          )}
        </View>
      </View>

      <View
        style={{
          backgroundColor: t.colors.glass,
          borderRadius: t.radius.lg,
          borderWidth: 1,
          borderColor: t.colors.border,
          padding: t.spacing.lg,
          gap: t.spacing.md,
        }}
      >
        <Text
          style={{
            color: t.colors.text,
            fontSize: 16,
            fontWeight: "800",
          }}
        >
          Links
        </Text>
        {editing && (
          <View
            style={{
              flexDirection: "row",
              gap: t.spacing.sm,
              flexWrap: "wrap",
            }}
          >
            <TextInput
              value={linkDraft.label}
              onChangeText={(v) =>
                setLinkDraft({ ...linkDraft, label: v })
              }
              placeholder="Rótulo (ex.: GitHub)"
              placeholderTextColor={t.colors.tabInactive}
              style={{
                flex: 1,
                minWidth: 120,
                backgroundColor: "#10152A",
                color: t.colors.text,
                borderRadius: t.radius.md,
                borderWidth: 1,
                borderColor: t.colors.border,
                paddingHorizontal: 12,
                height: 44,
              }}
            />
            <TextInput
              value={linkDraft.url}
              onChangeText={(v) =>
                setLinkDraft({ ...linkDraft, url: v })
              }
              placeholder="URL"
              placeholderTextColor={t.colors.tabInactive}
              autoCapitalize="none"
              style={{
                flex: 2,
                minWidth: 160,
                backgroundColor: "#10152A",
                color: t.colors.text,
                borderRadius: t.radius.md,
                borderWidth: 1,
                borderColor: t.colors.border,
                paddingHorizontal: 12,
                height: 44,
              }}
            />
            <TouchableOpacity
              onPress={addLink}
              style={{
                height: 44,
                paddingHorizontal: 14,
                borderRadius: t.radius.md,
                backgroundColor: t.colors.primary,
                alignItems: "center",
                justifyContent: "center",
              }}
            >
              <Plus color="#0B0D13" />
            </TouchableOpacity>
          </View>
        )}
        <View style={{ gap: t.spacing.sm }}>
          {links.map((l, i) => (
            <View
              key={`${l.label}-${i}`}
              style={{
                backgroundColor: "#1A2035",
                borderRadius: t.radius.md,
                borderWidth: 1,
                borderColor: t.colors.border,
                padding: t.spacing.md,
                flexDirection: "row",
                alignItems: "center",
                justifyContent: "space-between",
              }}
            >
              <TouchableOpacity
                onPress={() => Linking.openURL(l.url)}
                style={{
                  flexDirection: "row",
                  alignItems: "center",
                  gap: 8,
                  flex: 1,
                }}
              >
                <LinkIcon color={t.colors.primary} size={16} />
                <Text
                  style={{
                    color: t.colors.text,
                    fontWeight: "700",
                  }}
                >
                  {l.label}
                </Text>
                <Text
                  numberOfLines={1}
                  style={{
                    color: t.colors.textMuted,
                    marginLeft: 6,
                    flex: 1,
                  }}
                >
                  {l.url}
                </Text>
              </TouchableOpacity>
              {editing && (
                <TouchableOpacity
                  onPress={() => setLinks(removeAt(links, i))}
                >
                  <X color={t.colors.tabInactive} />
                </TouchableOpacity>
              )}
            </View>
          ))}
          {!links.length && (
            <Text style={{ color: t.colors.textMuted }}>Sem links</Text>
          )}
        </View>
      </View>

      <View
        style={{
          backgroundColor: t.colors.glass,
          borderRadius: t.radius.lg,
          padding: t.spacing.lg,
          borderWidth: 1,
          borderColor: t.colors.border,
          alignItems: "center",
          justifyContent: "center",
          gap: 6,
        }}
      >
        <Svg width={size} height={size}>
          <Circle
            cx={size / 2}
            cy={size / 2}
            r={r}
            stroke={t.colors.border}
            strokeWidth={stroke}
            fill="none"
          />
          <Circle
            cx={size / 2}
            cy={size / 2}
            r={r}
            stroke={t.colors.primary}
            strokeWidth={stroke}
            strokeDasharray={`${c * (completeness / 100)}, ${c}`}
            strokeLinecap="round"
            rotation="-90"
            originX={size / 2}
            originY={size / 2}
            fill="none"
          />
        </Svg>
        <Text
          style={{
            color: t.colors.text,
            fontSize: 22,
            fontWeight: "800",
            position: "absolute",
          }}
        >
          {completeness}%
        </Text>
        <Text
          style={{
            color: t.colors.textMuted,
            marginTop: t.spacing.xs,
          }}
        >
          Nível de completude do currículo
        </Text>

        {completeness < 100 ? (
          <View
            style={{
              marginTop: t.spacing.sm,
              alignSelf: "stretch",
              gap: 4,
            }}
          >
            <Text
              style={{
                color: t.colors.textMuted,
                marginBottom: 2,
              }}
            >
              Para chegar perto de 100%, você pode:
            </Text>
            {suggestions.slice(0, 3).map((sug, idx) => (
              <View
                key={idx}
                style={{
                  flexDirection: "row",
                  alignItems: "flex-start",
                  gap: 6,
                }}
              >
                <CheckCircle
                  color={t.colors.primary}
                  size={14}
                  style={{ marginTop: 2 }}
                />
                <Text
                  style={{
                    flex: 1,
                    color: t.colors.textMuted,
                  }}
                >
                  {sug}
                </Text>
              </View>
            ))}
          </View>
        ) : (
          <Text
            style={{
              color: t.colors.textMuted,
              marginTop: t.spacing.sm,
            }}
          >
            Seu currículo está bem completo. Agora foque em melhorar descrições
            e resultados.
          </Text>
        )}
      </View>

      <TouchableOpacity
        onPress={handleGeneratePdf}
        style={{
          alignSelf: "center",
          backgroundColor: t.colors.primary,
          paddingHorizontal: 18,
          paddingVertical: 12,
          borderRadius: t.radius.pill,
          flexDirection: "row",
          alignItems: "center",
          gap: 8,
        }}
      >
        <Download color="#0B0D13" />
        <Text style={{ color: "#0B0D13", fontWeight: "800" }}>
          Baixar PDF
        </Text>
      </TouchableOpacity>

      <View style={{ alignItems: "center", marginTop: t.spacing.lg }}>
        <Award color={t.colors.primary} size={22} />
        <Text
          style={{
            color: t.colors.textMuted,
            fontSize: 12,
            marginTop: 4,
          }}
        >
          R.I.S.E. • Requalificação, Inclusão, Sustentabilidade e Empregabilidade
        </Text>
      </View>
    </ScrollView>
  );
}
